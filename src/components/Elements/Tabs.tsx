    ...
    <ButtonContainer { ...props }
      ...
      }}>
      {
        values?.map((valueString) => <div key={valueString} class={{
          ...
          <button class="lum-btn-p-1 pr-0" onClick$={() => onClick$?.(valueString)}>
            {valueString}
          </button>
          <button class="lum-btn lum-bg-transparent hover:lum-bg-red p-0.5 rounded-lum-2 m-1" onClick$={() => onDelete$?.(valueString)}>
            <X size={16} />
          </button>
        </div>)
      }
