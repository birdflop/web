...
import { Eye, Pencil } from 'lucide-icons-qwik';
...
const InputField = component$(
  ({
    ...
    return (
      <div
        class={{
          'focus-within:border-lum-accent relative break-all': true,
          [`${className}`]: className,
      ...
      >
        ...
        {!readOnly && (
          <textarea
            ...
            value={combinedText(rgbSegments.value)}
            spellcheck={false}
            id={ADVANCED_INPUT_ID}
            onInput$={(e, el) => {
...
export default component$(({ readOnly }: { readOnly?: boolean }) => {
  ...
  return (
    <label for={ADVANCED_INPUT_ID} class="relative flex flex-col items-start">
      {previewStyle.value != 'default' && (
        <MCPreviewInput
          readOnly={readOnly}
          chatInput={generateAdvancedOutput(rgbSegments.value, rgbStore)}
        >
          {preview}
        </MCPreviewInput>
      )}
      {previewStyle.value == 'default' && (
        <DefaultInput readOnly={readOnly}>{preview}</DefaultInput>
      )}
      <div class="absolute top-1 right-1 flex items-center gap-1">
        ...
        <SelectMenuRaw
          ...
          customDropdown
          class={{ 'lum-grad-bg-lum-card-bg/75 rounded-lum-1 gap-1 p-1': true }}
        >
