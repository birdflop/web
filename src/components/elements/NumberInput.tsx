...
export const RawNumberInput = component$(({ id, value, input, onInput$, onDecrement$, onIncrement$, min, max, step }: any) => {
  return (
    <div class={`flex ${input ? '' : 'gap-2'}`}>
      <button data-action="decrement" disabled={value <= min} onClick$={onDecrement$} class={`transition ease-in-out bg-gray-${input ? 700 : 800} text-white text-2xl hover:bg-gray-${input ? 500 : 700} h-full py-1.5 ${input ? 'w-20 rounded-l-md' : 'w-[50%] rounded-md'} cursor-pointer`}>
          -
      </button>
      {
      ...
      <button data-action="increment" disabled={value >= max} onClick$={onIncrement$} class={`transition ease-in-out bg-gray-${input ? 700 : 800} text-white text-2xl hover:bg-gray-${input ? 500 : 700} h-full py-1.5 ${input ? 'w-20 rounded-r-md' : 'w-[50%] rounded-md'} cursor-pointer`}>
          +
      </button>
    </div>
  );
