...
const WORLD_MULT: Record<WorldSize, number> = {
  small: 0.9,
  medium: 1,
  large: 1.2,
};
...
export function planHardware(input: HardwareInput): HardwareResult {
  ...
  if (software === 'proxy') {
    ...
    return {
      ...
      cpuAdvice:
        'Proxies are mostly network-bound. Any modern CPU with 2 dedicated cores is plenty; prioritise low-latency networking and high single-thread clock.',
      storageGb: 2,
      ...
      breakdown: [
        { label: 'Base (proxy)', value: '0.5 GB' },
        {
          ...
          value: `${(players * 0.0025).toFixed(2)} GB`,
        },
      ],
      notes: [
  ...
  if (players <= 20) {
    cpuAdvice =
      'Single-thread clock is what matters. Any modern high-clock CPU (4.0 GHz+) handles this comfortably.';
  } else if (players <= 80) {
    cpuAdvice =
      'Prioritise a high single-thread CPU (e.g. Ryzen 7000/9000 series at 4.5 GHz+). The main game loop cannot be spread across cores.';
  } else {
    cpuAdvice =
      'You are pushing one server hard — pick the highest single-thread CPU available and consider splitting load across multiple servers behind a proxy.';
  }
  ...
  const storageGb = Math.ceil(
    jarAndLogs + worldBase + playerTerrain + viewPregen + pluginData
  );
  ...
  if (ram >= 24)
    notes.push(
      "Allocations above ~12 GB can cause longer garbage-collection pauses. Tuned GC flags (e.g. Aikar's) matter more here."
    );
  if (viewDistance >= 14)
    notes.push(
      'A view distance of 14+ dramatically increases RAM and bandwidth. Consider 8–10 with a plugin like Chunky for pre-generation.'
    );
  if (plugins > 0) {
    notes.push(
      modded
  ...
  notes.push(
    'This is only a general estimate — actual usage could be higher or lower. Monitor your own server with /spark and adjust.'
  );
  ...
  return {
    ...
    breakdown: [
      ...
      {
        label: `Plugins/mods (${plugins})`,
        value: `${pluginsRam.toFixed(2)} GB`,
      },
      {
        label: `View distance (${viewDistance})`,
        value: `${chunkRam.toFixed(2)} GB`,
      },
