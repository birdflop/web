...
import { colorFormats, rgbDefaults, generateOutput } from '@birdflop/rgbirdflop';
...
const descriptions: {
  ...
  colorFormat: 'The format to use for the color and format codes. For MiniMessage or JSON, { color: "MiniMessage" } can be used.',
...
const customTypes: {
  ...
  colorFormat: 'RegularFormatting | MiniMessageFormatting | JSONFormatting - see types in docs',
...
function getOutput(body: any) {
  ...
  if (format && !format.char && (!format.bold || !format.italic || !format.underline || !format.strikethrough)) {
    format = colorFormats.find((f: any) => f.color == format.color) ?? { ...format, char: '&' };
  }
