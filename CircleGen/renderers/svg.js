...
Renderer.Svg = function() {
	...
	this.render = function() {
		...
		for( var ix = 0; ix < _svg_width; ix += _dfull ) {
			text += '<rect x="' + (ix + (_dwidth / 2)) + '" y="0" fill="#EE3F3F" width="' + _dborder + '" height="' + _svg_height + '" opacity=".4" />';
		}

		for( var iy = 0; iy < _svg_height; iy += _dfull ) {
			text += '<rect x="0" y="' + (iy + (_dwidth / 2)) + '" fill="#EE3F3F" width="' + _svg_width + '" opacity=".6" height="' + _dborder + '"/>';
		}
